...
export default async function AdminDashboard() {
  ...
  const [
    ...
    db.order.count({
      where: { tenant_id: tenantId, createdAt: { gte: startOfDay } },
    }),
    db.order.count({
    ...
    db.variant
      .findMany({
        where: { tenant_id: tenantId, is_active: true, stock: { lte: 5 } },
        distinct: ["product_id"],
  ...
  const stats = [
    ...
    {
      label: "Total Revenue",
      value: `₹${Number(totalRevenue._sum.total ?? 0).toLocaleString("en-IN")}`,
      sub: "all time",
    },
    {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        {stats.map(({ label, value, sub, alert }) => (
          ...
          >
            ...
            <p
              className={`text-3xl font-bold mt-2 ${alert ? "text-amber-600" : "text-gray-800"}`}
            >
              {value}
            </p>
