...
export default async function ProductsPage({ searchParams }: Props) {
  ...
  const activeBrands = params.brands
  ...
  const [{ products, total, pages, priceRange }, categoryTree, brands] =
    await Promise.all([
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          ...
          {activeBrands.map((b) => (
            <a
              ...
              href={filterUrl({
                brands:
                  activeBrands.filter((x) => x !== b).join(",") || undefined,
              })}
              className="flex items-center gap-1.5 px-3 py-1 bg-gray-100 rounded-full text-xs text-gray-700 hover:bg-gray-200"
            >
          ...
          {(activeMin !== undefined || activeMax !== undefined) && (
            <a
              href={filterUrl({ min: undefined, max: undefined })}
              className="flex items-center gap-1.5 px-3 py-1 bg-gray-100 rounded-full text-xs text-gray-700 hover:bg-gray-200"
            >
              💰 ₹{activeMin?.toLocaleString("en-IN") ?? priceRange.min} – ₹
              {activeMax?.toLocaleString("en-IN") ?? priceRange.max}
              <span className="text-gray-400">×</span>
            </a>
          )}
          {activeInStock && (
            <a
              href={filterUrl({ instock: undefined })}
              className="flex items-center gap-1.5 px-3 py-1 bg-gray-100 rounded-full text-xs text-gray-700 hover:bg-gray-200"
            >
          ...
          {pages > 1 && (
            <div className="flex justify-center gap-2 mt-12 flex-wrap">
              ...
              {Array.from({ length: pages }, (_, i) => i + 1)
                .filter(
                  (p) =>
                    Math.abs(p - currentPage) <= 2 || p === 1 || p === pages,
                )
