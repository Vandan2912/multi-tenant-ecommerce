...
export default function CheckoutPage() {
    ...
    function validate() {
        ...
        if (!fields.phone.trim() || fields.phone.length < 10) e.phone = "Enter valid phone";
        if (!fields.line1.trim()) e.line1 = "Required";
        if (!fields.city.trim()) e.city = "Required";
        if (!fields.state.trim()) e.state = "Required";
        if (!fields.pincode.trim() || fields.pincode.length < 6) e.pincode = "Enter valid pincode";
        setErrors(e);
                ...
                <div className="md:col-span-2 space-y-8">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            <div className="sm:col-span-2">
                                <input placeholder="Address Line 1 *"
                                    value={fields.line1} onChange={(e) => set("line1", e.target.value)}
                                    className={inputClass("line1")} />
                                {errors.line1 && <p className="text-red-400 text-xs mt-1">{errors.line1}</p>}
                            </div>
                            <div className="sm:col-span-2">
                                <input placeholder="Address Line 2 (optional)"
                                    value={fields.line2} onChange={(e) => set("line2", e.target.value)}
                            ...
                            <div>
                                <input placeholder="Pincode *" maxLength={6}
                                    value={fields.pincode} onChange={(e) => set("pincode", e.target.value)}
                    ...
                    <section>
                        ...
                        <div className="grid sm:grid-cols-2 gap-3">
                            {enableCOD && (
                                <label className={`flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-colors ${fields.paymentMethod === "cod" ? "border-current" : "border-gray-200"
                                    }`}
                                    style={{ color: fields.paymentMethod === "cod" ? primaryColor : undefined }}>
                                    <input type="radio" name="payment" value="cod"
                                        checked={fields.paymentMethod === "cod"}
                                        onChange={() => set("paymentMethod", "cod")} className="hidden" />
                            ...
                            <label className={`flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-colors ${fields.paymentMethod === "online" ? "border-current" : "border-gray-200"
                                }`}
                                style={{ color: fields.paymentMethod === "online" ? primaryColor : undefined }}>
                                <input type="radio" name="payment" value="online"
                                    checked={fields.paymentMethod === "online"}
                                    onChange={() => set("paymentMethod", "online")} className="hidden" />
                                <span className="text-2xl">💳</span>
                                <div>
