...
import { genId, SIZE_ORDER, sortSizeValues } from "@/lib/options";
...
export function OptionTypeManager({
    ...
    async function handleDelete(id: string) {
        if (!confirm("Delete this option type? Products using it will lose this option.")) return;
...
function OptionTypeCard({
    optionType, isEditing, onEdit, onDelete, onUpdate,
}: {
    ...
    const [values, setValues] = useState<OptionValueMeta[]>(optionType.values_json);
    ...
    function addValue() {
        ...
        const label = isBoolean ? (values.length === 0 ? "Yes" : "No") : newVal.trim();
        if (values.find((v) => v.label.toLowerCase() === label.toLowerCase())) return;
                ...
                <div className="flex items-center gap-3">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${TYPE_BADGE[optionType.type] ?? "bg-gray-100 text-gray-600"}`}>
                                {optionType.type}
                            </span>
                        </div>
            ...
            {!isEditing && (
                <div className="px-6 py-3 flex flex-wrap gap-2">
                    {optionType.values_json.length === 0 ? (
                        <p className="text-sm text-gray-400">No values yet — click Edit values</p>
                    ) : optionType.values_json.map((v) => (
                        <span key={v.id} className="flex items-center gap-1.5 px-3 py-1 bg-gray-50 rounded-full text-sm text-gray-700">
                            {isColour && v.hex && (
                ...
                <div className="px-6 py-4 space-y-4">
                    ...
                    {isSize && (
                        <div className="flex flex-wrap gap-1.5">
                            ...
                            {["XS", "S", "M", "L", "XL", "XXL"].filter(
                                (s) => !values.find((v) => v.label === s)
                            ).map((s) => (
                                <button key={s}
                                    onClick={() => {
